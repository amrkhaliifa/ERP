...
rr.get("/outstanding", (req, res) => {
  const rows = db5
    .prepare(
      `SELECT o.id, c.name AS client, o.subtotal,
(o.deposit_paid + IFNULL((SELECT SUM(amount) FROM payments WHERE order_id=o.id),0)) AS paid,
(o.subtotal - (o.deposit_paid + IFNULL((SELECT SUM(amount) FROM payments WHERE order_id=o.id),0))) AS balance,
o.created_at
FROM orders o JOIN clients c ON c.id=o.client_id
WHERE (o.subtotal - (o.deposit_paid + IFNULL((SELECT SUM(amount) FROM payments WHERE order_id=o.id),0))) > 0
